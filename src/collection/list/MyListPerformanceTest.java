...
public class MyListPerformanceTest {
    ...
    public static void main(String[] args) {
        ...
        addMid(new MyArrayList<>(), size);      // 찾는데는 O(1), 데이터 추가(밀기) O(n)
        MyArrayList<Integer> arrayList = new MyArrayList<>();   // 조회용 데이터로 사용
        addLast(arrayList, size);     // 찾는데는 O(1), 데이터 추가(밀기) O(1)
        ...
        addMid(new MyLinkedList<>(), size);     // 찾는데는 O(n), 데이터 추가 O(1)
        MyLinkedList<Integer> linkedList = new MyLinkedList<>(); // 조회용 데이터로 사용
        addLast(linkedList, size);    // 찾는데는 O(n), 데이터 추가 O(1)
